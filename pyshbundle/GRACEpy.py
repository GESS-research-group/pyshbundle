...
def upwcon(degree: int, height: int) -> np.ndarray:
    ...
    Returns:
        (np.ndarray): Upward continuation terms.
    ...
    """
    rr = np.divide(GC.ae, np.add(GC.ae, height))
    uc = np.power(rr, degree)
...
def lovenr(lmax: int) -> np.ndarray:
    ...
    Returns:
        np.ndarray: LOVE number of degree lmax.
...
def lovenrPREM(lmax: int, frame: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    Returns:
        tuple: A tuple containing:
            - kn (np.ndarray): Load LOVE number of degree n.
            - hn (np.ndarray): LOVE number of degree n.
            - ln (np.ndarray): Shida number of degree n.
    ...
    l = data[:, 0]
    hl = data[:, 1]
    kl = np.divide(data[:, 2], l)
    ll = np.divide(data[:, 3], l)
