...
from pyshbundle.shutils import (
    normalklm,
    plm,
    iplm,
    eigengrav,
...
def gshs(field, quant="none", grd="mesh", n=-9999, h=0, jflag=1):
    ...
    if grd == "block" or grd == "cell":
        ...
        cshift = np.array([np.ones(nlat)], dtype="float").T * np.array(
            [np.cos(m * np.pi / 2 / n)], dtype="float"
        )  # cshift/sshift describe the
        sshift = np.array([np.ones(nlat)], dtype="float").T * np.array(
            [np.sin(m * np.pi / 2 / n)], dtype="float"
        )  # half-blocksize lambda shift.
...
def PhaseCalc(fts, ffts):
    ...
    for i in range(c):
        ...
        A = np.array(
            (
                np.ones((rn)),
                np.real(signal.hilbert(ffts[:, i])),
                np.imag(signal.hilbert(ffts[:, i])),
            )
        ).T
        ...
        ps[0, i] = np.arctan2(abc[3 - 1], abc[2 - 1]) * (
            180 / np.pi
        )  # check indices and degree/radian
    return ps
...
def GRACE_Data_Driven_Correction_Vishwakarma(F, cf, GaussianR, basins):
    ...
    Areahalfdeg = (
        (6378.137**2)
        * (((np.pi / 180) * lambdd1) - ((np.pi / 180) * lambdd))
        * (np.sin((np.pi / 2) - theta_rad) - np.sin((np.pi / 2) - theta1_rad))
    )
    ...
    if type(basins) != np.ndarray:
        raise Exception(
            "input basin field should be in Numpy NdArray format, please check guidelines"
        )
    ...
    if l == cfield:
        for m in range(r):
            ...
            ffFld__, _, _ = gshs(
                (Ft * filter_ * filter_), qty, "cell", int(180 / deg), 0, 0
            )

            if m == 0:
                fFld = np.zeros((r, fFld__.shape[0], fFld__.shape[1]), dtype="double")
                ffFld = np.zeros(
                    (r, ffFld__.shape[0], ffFld__.shape[1]), dtype="double"
                )
    ...
    for rbasin in range(0, cid):
        ...
        for m in range(0, r):
            ...
            if np.isnan(
                fF[:20, :20]
            ).any():  # if there is a gap in time series, fill it with NaNs
                ...
                tsleaktotalff[m][rbasin] = np.sum(ffF * kappa * Area) / np.sum(
                    Rb * Area
                )
                ...
                bfDevRegAv[m][rbasin] = np.sum(
                    (fF * Rb - FilteredTS[m][rbasin]) * filRb * Area
                ) / np.sum(Rb * Area)  # working 2022-10-20
                bbfDevRegAv[m][rbasin] = np.sum(
                    (ffF * Rb - filfilts[m][rbasin]) * filRb * Area
                ) / np.sum(Rb * Area)
                print(m)
    ...
    for i in range(0, cid):
        ...
        A[:, 1] = naninterp(
            bbfDevRegAv[:, i]
        )  # Pchip interpolate should contain atleast two elements

        lssol_ = sc.linalg.lstsq(
            A, naninterp(bfDevRegAv[:, i])
        )  # returns a tuple of solution "x", residue and rank of matrix A; for A x = B
        lssol = lssol_[0]
    ...
    for i in range(0, cid):
        ftsleaktotal[:, i] = naninterp(
            tsleaktotalf[:, i]
        )  # Replaces gaps (NaN values) with an itnerpolated value in the leakage time series from once filtered fields
        fftsleaktotal[:, i] = naninterp(
            tsleaktotalff[:, i]
        )  # replace the gaps (NaN values) with an interpolated value in leakage time series from twice filtered fields

        X = sc.fft.fft(
            ftsleaktotal[:, i]
        )  # take fast Fourier transform #check shape of X 2022-10-21
        p = (
            -ps[0, i] / r
        )  # compute the fraction of the time period by which the time series is to be shiftes
        Y = np.exp(
            1j * np.pi * p * ((np.arange(r)) - r / 2) / r
        )  # compute the Conjugate-Symmetric shift
    ...
    lhat = (
        leakager * rfn
    )  # apply the amplitude ratio to the shifted leakage timeseries from the once filtered fields to get the near true leakage
