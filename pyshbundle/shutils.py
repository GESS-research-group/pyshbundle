...
def plm(l: np.array, m: int, thetaRAD, nargin, nargout):
    ...
    if nargout == 3:  # second derivative needs also dP_{n,m+1} and dP_{n,m-1}
        ...
        ptmp_m2 = np.zeros(
            (n, lmax + 4 - m), dtype="float"
        )  # but these first derivative need dP_{n,m+2} and dP_{n,m-2}
...
def derivALF(inn, miin, plin, m, lmax):
    ...
    if m == 0:
        ...
        if lmax > m:
            inn[:, 1:] = plin * (
                -np.sqrt(((l[1:] + 1) * l[1:] / 2).real)
            )  # (-ones(n,1)*realsqrt((l(2:end)+1).*l(2:end)./2)).*plin
    elif m == 1:
        inn[:, 0] = miin[:, 1]
        if lmax > m:
            inn[:, 1:] = miin[:, 2:] * (
                np.sqrt((l[1:] + 1) * l[1:] / 2).real
            ) - 0.5 * plin * (np.sqrt((l[1:] - 1) * (l[1:] + 2)).real)
    elif m == lmax:
        inn[:, 0] = np.sqrt(m / 2 * miin[:, 1:]).real
    else:
        inn[:, 0] = np.sqrt((m / 2) * miin[:, 1:]).real
        if lmax > m:
            inn[:, 1:] = 0.5 * miin[:, 2:] * np.sqrt(
                (l[:, 1:] + m) * (l[:, 1:] - m + 1)
            ).real - 0.5 * plin * (np.sqrt((l[:, 1:] - m) * (l[:, 1:] + m + 1)).real)
    return inn
...
def ispec(a, b=-9999):
    ...
    if (np.absolute(b[n2 - 1, :]) < 1e-10).all():
        ...
        fs = (a - 1j * b) / 2
        fs = (
            np.concatenate((fs, np.conj(fs[np.arange(n2 - 2, 0, -1), :])), axis=0)
        ) * max(n, 1)
        ...
        fs = (a - 1j * b) / 2
        fs = (
            np.concatenate((fs, np.conj(fs[np.arange(n2 - 1, 0, -1), :])), axis=0)
        ) * n
...
def eigengrav(lmax: int, fstr: str, h: float):
    ...
    if fstr == "none":
        ...
        tf = np.multiply(range(1, (lmax + 2), 1), range(2, (lmax + 3), 1)) * (
            (GC.GM / r / r) * 1e9
        )
        tf = tf.reshape((1, lmax + 1))
    elif fstr == "slope":
        tf = np.sqrt(np.multiply(range(0, lmax + 1, 1), range(1, lmax + 2, 1)))
        tf = tf.reshape((1, lmax + 1))
    elif fstr == "water":
        ln = GB.lovenr(lmax)
        tf = np.divide(
            np.multiply(5.517 * r, np.add(range(0, 2 * lmax + 1, 2), 1)),
            np.multiply(3, (1 + ln)),
        )
        tf = tf.reshape((1, lmax + 1))
    elif fstr == "smd":
        ln = GB.lovenr(lmax)
        tf = np.divide(
            np.multiply(5517 * r, np.add(range(0, 2 * lmax + 1, 2), 1)),
            np.multiply(3, (1 + ln)),
        )
        tf = tf.reshape((1, lmax + 1))
    elif fstr == "height":
...
def grule(n: int):
    ...
    bp = -x0 - h
    fx = d1 - h * e1 * (
        pk + (h / 2) * (dpn + (h / 3) * (d2pn + (h / 4) * (d3pn + (0.2 * h) * d4pn)))
    )
...
def Gaussian(L: int, cap: int):
    ...
    W[0, 0] = 1
    W[1, 0] = np.power(
        10, np.log10((1 + np.exp(-2 * b)) / (1 - np.exp(-2 * b)) - (1 / b))
    )
